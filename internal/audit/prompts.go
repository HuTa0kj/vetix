...
// OutputLanguage 复刻 Python 的 get_output_language：只影响 LLM 生成的 finding
// 文本，不改日志、表头或 JSON 键名。
...
// VerifyPrompt 构造复核提示词。命中列表用 Python dataclass 的 repr 文本，
// 路径保留 Python 版硬编码的 "/" 前缀——提示词是在这套文本上调优的。
...
func BehavioralPrompt(s *State) string {
	// Python 版在绝对路径前又加了一个 "/"，实际文本形如 //Users/...。这是既有的
	// 畸形拼接，提示词在那段文本上调优过，保持原样。
...
// TreeRepr 渲染成 Python dict 的 repr：单引号、多一层 "." 根、
// {'line_count': N} 叶子都要保留，因为它们就是模型看到的"目录结构"。
