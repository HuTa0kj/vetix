...
// 只读与白名单检查。两者的组合才等价于 Python 的 ToolFilterMiddleware。
...
//
// eino 的默认行为是把工具错误当成节点失败向上抛，整轮 agent 直接终止；Python 的
// LangChain 会把错误作为 tool message 交回模型自行纠正。对只读文件工具来说终止
// 完全没有必要——grep 传了 RE2 不支持的正则、读了不存在的路径，都是模型自己就能
// 改的。这里统一降级成可自纠的文本结果，并保留原始原因。
